...
def init_bit_mapping(n, algo=None, func=None):
    ...
    if algo is Algos.DJ:
        ...
        oracle_map = {}
        # alpha and beta given as parameters
        a, b = func
        # for every qubit string x => calculate (a*x + b) % 2
        for q in qubits:
            dp = b
...
def gen_matrix(f, n, m):
    ...
    U_f = Operator(np.zeros((2**(n+m),2**(n+m))))
    ...
    for xb in range(0, int(2**(n+m))):
        ...
        xv = Operator(np.zeros((2**n,1)))
        xv.data[int(x, 2)] = 1.
        bv = Operator(np.zeros((2**m,1)))
        bv.data[int(b, 2)] = 1.
        bfxv = Operator(np.zeros((2**m,1)))
        bfxv.data[int(bfx, 2)] = 1.

        # Accumulate (|x><x| (*) |b + f(x)><b|) into the sum
        # (*) is the tensor product
        U_f += (xv * xv.transpose()).tensor(bfxv * bv.transpose())
